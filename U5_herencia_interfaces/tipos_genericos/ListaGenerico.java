...
public class ListaGenerico<T> {
  ...
  public ListaGenerico(int capacidad) {
  ...
  public boolean insertar_en_posicion(int n, T x) {
  ...
  public void insertar_principio(T x) {
    insertar_en_posicion(0, x);
  }

  public void insertar_final(T i) {
    insertar_en_posicion(num_elementos, i);
  }

  // Elimina el elemento de la posición n
  public T eliminar_elemento(int n) {
  ...
  public T obtener_elemento(int n) {
  ...
  public int buscar_elemento(T x) {
    int resultado = -1;
  ...
  public void insertar_lista_final(ListaGenerico<T> l2) {
    t = Arrays.copyOf(t, t.length + l2.get_num_elementos());

    for (int i = 0; i < l2.get_num_elementos(); i++) {
      insertar_final((T) l2.obtener_elemento(i));
    }
  }
